...
#
# Currently* supported Linux distributions for Navencrypt:
#   - Ubuntu 12.04
#   - Amazon Linux 2013.09
#   - RHEL/CentOS 5.9+ (untested), 6.x
#
# * Please check Navencrypt product page for full listing of supported distros.
...
log_file="/tmp/navencrypt-install.log"
...
# Install Navencrypt prerequisites for the system.
...
# Install Navencrypt.
function install_navencrypt {
    printf "Checking for navencrypt.\n"
    case "$operating_system" in
        "centos" | "redhat" | "oracle" )
        rpm -q navencrypt &>$log_file && printf "\t- Already installed.\n" && return 0
        printf "\t- Not present. Installing Navencrypt with yum.\n" && yum install navencrypt -y &>$log_file || err "Navencrypt could not be installed. Stopping."
        chkconfig --level 2345 navencrypt-mount on &>$log_file
        chkconfig --level 2345 haveged on &>$log_file
        ;;
        "ubuntu" | "debian" )
        dpkg --list | grep navencrypt &>$log_file && printf "\t- Already installed.\n" && return 0
        printf "\t- Not present. Installing Navencrypt with apt-get.\n" && apt-get install navencrypt -y &>$log_file
        ;;
        "amazon" )
        rpm -q navencrypt &>$log_file && printf "\t- Already installed.\n" && return 0
        # Begin hack
        ...
        rm -f *.rpm &>$log_file
        # End hack
        printf "\t- Installing Navencrypt with yum.\n" && yum install navencrypt -y &>$log_file || err "Navencrypt could not be installed. Stopping."
        chkconfig --level 2345 navencrypt-mount on &>$log_file
        chkconfig --level 2345 haveged on &>$log_file
        ;;
        * )
        err "Invalid package manager (install_navencrypt)."
        ;;
    esac
    which navencrypt &>$log_file || err "Navencrypt could not be installed (install_navencrypt)."
    return 0
}
...
# Remove navencrypt-ping cron job.
function remove_navencrypt_ping {
    if [[ -f /etc/cron.hourly/navencrypt-ping ]]; then
        rm -f /etc/cron.hourly/navencrypt-ping &>$log_file
    fi
}

# Check status of kernel module. If not present, build it.
function check_zncryptfs {
    which navencrypt-module-setup &>$log_file || err "Navencrypt not installed (check_zncryptfs). Stopping."
    printf "Checking for Navencrypt kernel module.\n" && test -f /var/lib/dkms/zncryptfs/3*/$(uname -r)*/$(uname -i)/module/*.ko
    if [[ $? -ne 0 ]]; then
        printf "\t- Not found. Building Navencrypt kernel module.\n" && navencrypt-module-setup
        if [[ $? -ne 0 ]]; then
            err "Navencrypt kernel module (zncryptfs) did not build correctly."
        fi
    else
...
# Start the haveged service. This aids in secure key generation during Navencrypt's registration phase.
...
function stop_selinux {
    ...
    if [[ -f /etc/selinux/config ]]; then
        ...
        if [[ $? -eq 0 ]]; then
            sed -i.before_navencrypt s/SELINUX=enforcing/SELINUX=disabled/ /etc/selinux/config && printf "\t- Modified configuration from enforcing to disabled.\n"
        fi
    fi
}
...
function main {
    ...
    install_navencrypt || err "Could not install Navencrypt. Please check logs for more detail."
    remove_navencrypt_ping
    check_zncryptfs
    start_haveged
    stop_apparmor
    stop_selinux
